...
(function() {
    ...
    function config(remoteActionsProvider, $logProvider, $compileProvider, $locationProvider) {
        var actions = ['drPreviewExecute', 'GetFieldsForObject', 'DeleteMappings', 'SaveMapping',
                        ...
                        'cloneDRBundle', 'createNewVersionDRBundle', 'getDefaultBundleForInterface', 'GenerateMappings',
                        ...
                        'getAllSObjectFieldsByReferenceToPath', 'getRelationsByPath', 'getDocuSignFieldMap', 'updateDRIsManagedUsingStdDesigner'];
...
DRMapperController2.$inject = ['DRBundle', 'fieldMetadata', 'fieldSetMetadata', '$scope',
                                 '_', 'remoteActions', '$q', 'DocumentService', '$sldsPrompt', '$sldsToast', 'mappingService', '$localizable', '$sldsModal', 'IsFoundation', 'isOmniMetadataApiEnabled', 'isDataRaptorVersioningEnabled', 'IsFileBased'];
...
function DRMapperController2(DRBundle, fieldMetadata, fieldSetMetadata, $scope,
    ...
    this.bundle.isFileBased = IsFileBased;
    this.isReadonly = (this.bundle.isFileBased) || (this.bundle.IsActive && isDataRaptorVersioningEnabled);
    ...
    if (this.bundle.Id == null && IsFoundation){
        this.bundle.VersionNumber = 1;
    }

    if (this.bundle.Id == null || isOmniMetadataApiEnabled){
        updateValidations();
    ...
    function updateValidations() {
        ...
        self.nameMaxLength = METADATA_API_ENABLED_NAME_MAX_LENGTH;
        self.namePattern = /^[a-zA-Z0-9]+$/;
    ...
    this.fields['VersionNumber'].hidden = !isDataRaptorVersioningEnabled;
    ...
    $scope.$createClone = function(newName) {
        return remoteActions.cloneDRBundle(IsFoundation ?  self.bundle.Id : self.bundle.Name, newName)
            .then(function(newId) {
                // Log Data Raptor Clone action
                try {
                    var logPayload = {
                        componentType: 'DataRaptor',
                        componentId: self.bundle.Id,
                        action: 'CLONE_DATARAPTOR',
                        moduleName: 'BUILDER_OMNISTUDIO_DATARAPTOR',
                        featureName: 'SANDBOX_USAGE',
                        context: JSON.stringify({
                            'IsFileBased': !!IsFileBased,
                            'IsCoreDesigner': !!IsFoundation,
                            'originalName': self.bundle.Name,
                            'newName': newName,
                            'newId': newId,
                            'dataRaptorType': self.bundle.Type,
                            'version': self.bundle.VersionNumber || '1.0'
                        })
                    };
                    // Call logging via remote action if available
                    if (remoteActions && remoteActions.logDataRaptorAction) {
                        remoteActions.logDataRaptorAction(JSON.stringify(logPayload));
                    }
                } catch (e) {
                    // Silent fail for logging
                    console.log('DataRaptor clone logging failed:', e);
                }
    ...
    function fixUpUrlWithParams(url) {
        ...
        if ('id' in combinedSearchParams && 'uniqueName' in combinedSearchParams) {
            delete combinedSearchParams.uniqueName;
        }
    ...
    const buttons = [
        {
            ...
            type: 'export',
            isDisabled: this.bundle.isFileBased,
            drvDataPackType: 'DataRaptor'
        },        {
            label: 'Activate',
            isDisabled: this.bundle.isFileBased,
            action: function() {
                self.bundle.IsActive = true;
                mappingService.saveBundle(self.bundle).then(function(result) {
                    // Log Data Raptor Activate action
                    try {
                        var logPayload = {
                            componentType: 'DataRaptor',
                            componentId: self.bundle.Id,
                            action: 'ACTIVATE_DATARAPTOR',
                            moduleName: 'BUILDER_OMNISTUDIO_DATARAPTOR',
                            featureName: 'SANDBOX_USAGE',
                            context: JSON.stringify({
                                'IsFileBased': !!IsFileBased,
                                'IsCoreDesigner': !!IsFoundation,
                                'dataRaptorName': self.bundle.Name,
                                'dataRaptorType': self.bundle.Type,
                                'version': self.bundle.VersionNumber || '1.0',
                        ...
                        // Silent fail for logging
                        console.log('DataRaptor activate logging failed:', e);
                    }
            ...
            hide: function () {
                return self.bundle.IsActive || (!isDataRaptorVersioningEnabled);
            }
        },
        {
            label: 'Deactivate',
        ...
        },
        {
            ...
            action: function() {
                remoteActions.createNewVersionDRBundle(self.bundle.Id)
                .then(function(newId) {
                    // Log Data Raptor New Version action
                    try {
                        var logPayload = {
                            componentType: 'DataRaptor',
                            componentId: self.bundle.Id,
                            action: 'NEW_VERSION_DATARAPTOR',
                            moduleName: 'BUILDER_OMNISTUDIO_DATARAPTOR',
                            featureName: 'SANDBOX_USAGE',
                            context: JSON.stringify({
                                'IsFileBased': !!IsFileBased,
                                ...
                                'dataRaptorName': self.bundle.Name,
                                'dataRaptorType': self.bundle.Type,
                                'originalVersion': self.bundle.VersionNumber || '1.0',
                                'newId': newId,
                                'originalId': self.bundle.Id
                            })
                        };
                        // Call logging via remote action if available
                        if (remoteActions && remoteActions.logDataRaptorAction) {
                        ...
                        // Silent fail for logging
                        console.log('DataRaptor new version logging failed:', e);
                    }
            ...
            hide: function () {
                return !isDataRaptorVersioningEnabled;
            }
        }
    ];
    ...
    if (typeof sforce !== 'undefined') {
        if (sforce.console && sforce.console.isInConsole()) {
            sforce.console.setTabTitle(self.bundle.Name || 'New Data Mapper');
            if (!self.bundle.Id) {
...
function ExtractFieldRowController(mappingService, $timeout, $element, $scope) {
    ...
    function onInit() {
        vm.editing = !vm.mapping.Id && !vm.isReadonly;
...
PreviewExtractController.$inject = ['$scope', 'remoteActions', '$location', '$q', 'vkbeautify', '_', 'mappingService', 'isDataRaptorVersioningEnabled'];

function PreviewExtractController($scope, remoteActions, $location, $q, vkbeautify, _, mappingService, isDataRaptorVersioningEnabled) {
    ...
    function submitRequest() {
        ...
        var params = {
            bundleName: (isDataRaptorVersioningEnabled && !vm.bundle.IsActive && vm.bundle.Id) ? vm.bundle.Id : vm.bundleName,
...
PreviewLoadTransformController.$inject = ['$scope', '$q', '$location', 'remoteActions', 'vkbeautify', '_', 'mappingService', 'isDataRaptorVersioningEnabled'];

function PreviewLoadTransformController($scope, $q, $location, remoteActions, vkbeautify, _, mappingService, isDataRaptorVersioningEnabled) {
