...
angular.module('ouihome')
    .controller('iphome', ['$scope', '$rootScope', '$drvExport', '$sldsPrompt', 'remoteActions',
        ...
        function ($scope, $rootScope, $drvExport, $sldsPrompt,
            ...
            // Helper method to sanitize Angular expressions to prevent XSS
            function sanitizeAngularExpression(input) {
                if (!input) return input;
                // Escape Angular expression delimiters to prevent template injection
            ...
            $scope.defaultColumns = [
                {
                    ...
                    additionalFields: window.isFoundationPackage ? [OmniScriptFieldMappingsNamespace.SubType__c, OmniScriptFieldMappingsNamespace.Version__c, OmniScriptFieldMappingsNamespace.IsManagedUsingStdDesigner] :
                    [OmniScriptFieldMappingsNamespace.SubType__c, OmniScriptFieldMappingsNamespace.Version__c],
                    getInfo:  function ($scope, row) {
                        // Only show tooltip when flag is disabled and IsManagedUsingStdDesigner is true (component is not clickable)
                        if (window.isOmniStudioHybridEnabled !== true && row[OmniScriptFieldMappingsNamespace.IsManagedUsingStdDesigner]) {
                            return window.i18n['IpCoreUpdatedComponent'];
                        }
                    },
                    getValue: function ($scope, row) {
                      ...
                      if (window.isOmniStudioHybridEnabled !== true && row[OmniScriptFieldMappingsNamespace.IsManagedUsingStdDesigner]) {
                          return '<span title="'+ _.escape(window.i18n['IpCoreUpdatedComponent']) +'">' + sanitizeAngularExpression(_.escape(row.Name)) + ' (Version ' + row[OmniScriptFieldMappingsNamespace.Version__c] + ')' + '</span>';
                      }
                      // If flag is enabled and IsManagedUsingStdDesigner === true, route to core builder
                      if (window.isOmniStudioHybridEnabled === true && row[OmniScriptFieldMappingsNamespace.IsManagedUsingStdDesigner]) {
                          url = '/builder_industries_interaction_rule/industriesBuilder.app?recordId=' + row.Id;
                          return '<a href="' + url + '" target="_blank" onclick="window.open(\'' + url + '\', \'_blank\'); return false;">' +
                              sanitizeAngularExpression(_.escape(row.Name)) + ' (Version ' + row[OmniScriptFieldMappingsNamespace.Version__c] + ')</a>';
                      }
                    ...
                    .then(function (response) {
                        ...
                        if (/integrationprocedurehome/.test(responseObj.url)) {
                            $scope.$evalAsync(function () {
                                ...
                                // Log Integration Procedure Activate/Deactivate action
                                try {
                                    var logPayload = {
                                        componentType: 'IntegrationProcedure',
                                        componentId: row.Id,
                                        action: isActivate ? 'ACTIVATE_IP' : 'DEACTIVATE_IP',
                                        moduleName: 'BUILDER_OMNISTUDIO_IP',
                                        featureName: 'SANDBOX_USAGE',
                                        context: JSON.stringify({
                                            'IsFileBased': !!row.IsFileBased,
                                            'userProfile': '',
                                            'IsCoreDesigner': !!window.isFoundationPackage,
                                            'ipName': row.Name,
                                            'ipType': row[OmniScriptFieldMappingsNamespace.Type__c],
                                            'ipSubType': row[OmniScriptFieldMappingsNamespace.SubType__c],
                                            'version': row[OmniScriptFieldMappingsNamespace.Version__c] || '1.0',
                                            'previousState': isActivate ? 'Inactive' : 'Active',
                                            'newState': isActivate ? 'Active' : 'Inactive'
                                        })
                                    };
                                    // Log via console for now since IP doesn't have direct logAction access
                                    console.log('Integration Procedure ' + (isActivate ? 'Activate' : 'Deactivate') + ' action logged:', logPayload);
                                } catch (e) {
                                    // Silent fail for logging
                                    console.log('IP activation logging failed:', e);
                                }
                            });
                        } else {
            ...
            $scope.rowActions = [
                ...
                },
                {
                    ...
                    promptContent: function (row) {
                        ...
                        sanitizeAngularExpression(_.escape(row.Name)) + ' (Version ' + row[OmniScriptFieldMappingsNamespace.Version__c] + ')" ';
                    },
                    hide: function (row, group) {
                        return row[OmniScriptFieldMappingsNamespace.IsActive__c];
                    }
