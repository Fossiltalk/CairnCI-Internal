...
/**
 * The default runner: spawn the real `sf` CLI. Every caller goes through
 * `createSfClient({ run })` rather than importing this directly, so tests can
 * substitute a stub without an org, a network, or the CLI on PATH.
 *
 * Resolves with the parsed `result` payload; rejects with an Error whose
 * message carries the CLI's own text — the retrieve phase pattern-matches that
 * message for unsupported-type detection (see retrieve-metadata.mjs).
 */
export function spawnSf(args, { timeout, cwd } = {}) {
...
/**
 * Builds the `sf` facade every phase uses. `run(args, {timeout, cwd})` is the
 * single seam: the real implementation shells out, tests pass a stub.
 *
 * @param {{ run?: typeof spawnSf }} [options]
 */
export function createSfClient({ run = spawnSf } = {}) {
  /** Run an `sf` subcommand, retrying transient failures. */
  async function sf(args, { retries = 2, timeout = 10 * 60 * 1000, cwd } = {}) {
    let lastError;
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        return await run(args, { timeout, cwd });
      } catch (err) {
        lastError = err;
  ...
  return {
    ...
    async query(targetOrg, soql, { toolingApi = false } = {}) {
    ...
    // written) — confirmed by hand against a real org. `--target-metadata-dir`
    ...
    retrieve(targetOrg, manifestPath, outputDir, { cwd, waitMinutes = 60 } = {}) {
      const args = [
        'project',
        'retrieve',
        'start',
    ...
    // `stagingRoot` in paths.mjs) or the CLI silently converts zero
    // components. `outputDir` (e.g. the repo's `force-app`) can be inside the
    // project — the CLI appends `main/default` to it itself.
    convertMdapi(rootDir, outputDir, { cwd } = {}) {
